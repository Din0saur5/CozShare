...
const UserRow = ({user, currentUser, setCurrentUserData, setList, list, table, viewedProfile}) => {
   ...
   // console.log(user)
    const [userView, setViewData] = useState(user)
  ...
  <Link to={followStatus === 'currentUser'? '/profile': `/profile/${userView.id}`}> 
  <div className="avatar hover:bg-transparent ">
                <div className="w-8 mask mask-hexagon hover:bg-transparent">
                    <img src={userView.profile_pic} onError={handleImgError} />
                </div>
    </div>
  <h2 className="card-title">{userView.display_name}</h2></Link>
    <p className='card-body p-0' >{userView.catchphrase}</p> 
    <div className="card-actions justify-end">
        {followStatus === 'currentUser' || table === 'query'? (<></>):(
            
            <button onClick={()=>{handleFollow(userView.id)}} className="btn btn-secondary btn-xs">{followStatus? 'Unfollow':'Follow' }</button>
        )}
    </div>
  </div>
</div>
